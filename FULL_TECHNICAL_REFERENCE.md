...
* **Dynamic Mount Management:** Intelligently assesses if the jail needs `/proc` and `/dev/pts` based on PHP-FPM settings and active Jailkit users. Mounts them securely using `hidepid=2`. Mounts survive server restart, because they are safely and idempotently maintained in `/etc/fstab` by the plugin.
