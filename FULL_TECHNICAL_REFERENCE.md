...
* **Git Server IP & SSH Routing (`PADM_GIT_SERVER_IP_ADDRESS`):** In `scripts/padm_shelluser_provision.sh`, this variable controls the SSH configuration generated for the Vault.
  ...
  * **Note on HTTPS:** HTTPS cloning is completely independent of this SSH routing logic. It will work regardless of which option you choose. However, you are entirely responsible for handling HTTPS authentication (public repositories will clone out-of-the-box without authorization).
