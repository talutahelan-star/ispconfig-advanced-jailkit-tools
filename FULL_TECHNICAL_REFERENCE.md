...
* **Git Server IP & SSH Routing (`PADM_GIT_SERVER_IP_ADDRESS`):** In `scripts/padm_shelluser_provision.sh`, this variable controls the SSH configuration generated for the Vault.
  * **Option 1 (Universal SSH - Default):** Leave it empty (`""`) to enable out-of-the-box compatibility with any DNS-resolvable Git provider (e.g., `ssh://git@github.com`, GitLab, Bitbucket, or standard self-hosted instances).
  * **Option 2 (Private IP-Only Servers):** Set an IP *only* if you are using a self-hosted Git server without a DNS hostname. If you do this, you can also customize the `GitServer` alias within the script's generation block. *Warning:* Choosing this option will server-wide cripple the ability to clone via SSH from standard DNS-based Git servers.
  * **Note on HTTPS:** HTTPS cloning is completely independent of this SSH routing logic. It will work regardless of which option you choose. However, you are entirely responsible for handling HTTPS authentication (public repositories of course will clone out-of-the-box without any authorization).
...
* **Identity Management:** Your Git identity (`login username`, `user.name`, `user.email`) is automatically populated and managed from a single centralized file, that the user can tune and change additionally post-provisioning.
...
2. For the `padm_shelluser_provision.sh` script, to debug and tune it, you can execute it from the terminal directly by passing your testing arguments. It's easy, just execute the script once without any arguments, and it will dump a "copy-paste-friendly" execution command, with sample arguments for you to tune accordingly. And then paste the command, and execute it at will, as many times as you want, for any shell-user you want. The script is idempotent. The script is agnostic to what the cwd is, it always plays with absolute paths, also there are safe-checks implemented in the logic before every "delete" command, so don't worry too much before executing it. Remember: this script is designed to be executed as root.
3. Debugging `/etc/jailkit/jk_init.ini` changes is more difficult. You need to recreate the website to fully reload a particular jail. Jails are per website. One website -> only one jail. Changing appsections on ISPConfig UI -> System -> Server Config -> Jailkit tab isn't reloading any jails on the server at all. A Full delete, then create of a website is required if one needs to test that. Changes on appsections on Website-level executes immediately (Sites -> Website -> example10.com -> Options tab - Jailkit chroot app sections), however, it applies only incrementally (if you add an appsection to the list, it will add the programs to the jail). But if you remove an appsection, it never "downgrades" the jail. For a downgrade-test, you need again to re-create the entire website. Appsections imposed on Website-level simply overrides non-incrementally (completely replaces) the appsections imposed on Server Config level. But again -> no automatic downgrades of the Jail are being performed by the system. Programs are added, to the jail, and they are never getting removed until a full-recreation of a website. And this is obvious, because re-creation of a website deletes the Jail, and creates a completely new one.
