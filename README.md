...
* **Non-jailed shell-user:** A creation of at least one Non-jailed shell-user (per website) is required for the particular feature of "Detached Git Tracking" to work for that website.
* **Detached Git Tracking:** Detached Git tracking and deployments in the current version was written to work with my own custom remote self-hosted Git Service (Forgejo), having NO hostname :) Hence this is the reason you will see a hardcoded IP configuration in `scripts/padm_shelluser_provision.sh`. However, there is no reason why passing `https://github.com/joomla/joomla-cms.git` or `ssh://git@github.com:octobercms/october.git` as an argument shouldn't work. For now just ignore the warnings that say: "These helper scripts only work with ForgejoGit". Those warnings will be completely removed in future versions, I guess soon. You can leave the hardcoded IP unchanged in `scripts/padm_shelluser_provision.sh` (var `PADM_GIT_SERVER_IP_ADDRESS`). This IP becomes irrelevant, unless you intend to clone from a similar self-hosted Git service of yours (e.g: without a hostname), in that case you would need to input your IP there. Generally using `ssh://../...git` cloning is preferable (instead of https://), because it's easier for you to setup and more secure. Easier because all your keys are getting automatically generated by the extension scripts, along with configurable Git username, user.email and user.name, managed in one file. Everything stored as x00 mode, in a secure vault dir outside the Jail-chroot of the website.
...
6. **Secure Dynamic Mounts (`/proc` & `/dev/pts`):** The plugins intelligently provide essential virtual filesystems to the jail *only* when authorized, using strict anti-snooping principles. Jailed users are restricted to seeing only their own background processes, and they receive a completely private, isolated terminal space. This safely enables utilities like `dtach` and complex Java or Node.JS terminal-IO apps to function, while making it mathematically impossible for users to spy on or hijack each other's sessions. This is very good for isolating AI agents working with Remote-SSH who at the same time has a viable environment to work upon.
