...
6. **Dynamic Mount Management (`/proc` & `/dev/pts`):** The plugins intelligently bind-mount essential virtual filesystems into the jail *only* when authorized (via the `ps_top_w_uptime` appsection). This securely allows users to view their own processes via `ps` and `top`, and provisions pseudo-terminals (`/dev/pts`) strictly required for complex background utilities like `dtach` and the Java JRE to function without terminal allocation errors.
