...
4. **Context-Aware Terminal:** Custom bash prompts that dynamically display your active Git branch (in cyan) *only* when you are inside the `/web` directory.
5. **Background Sessions (`dtach`):** Run persistent background programs in your jails using custom, secure wrapper commands.
6. **Smart Identity & Environment Context:** Automatically extracts your website domain, client email, and contact details from the ISPConfig database to personalize your terminal prompts and pre-configure your Git authorship without manual setup.
